...
library (stringr)
#Generando nombres de variables, s�lo mean() y std()
varnames = read.table("features.txt", sep ="")
x = str_split_fixed(varnames$V2, pattern = "-", n = 3)
y = paste(x[,1], x[,2], sep = " ")
m = x[x[,2]=="mean()" | x[,2]=="std()",]
col = paste(m[,1], m[,2])
#fusionando los datos del grupo test y train, s�lo mean() y std()
...
ytrain[1,1] = 5 #para corregir caracteres extra�os
ytest[1,1] = 5 #para corregir caracteres extra�os
...
tidydata = apply(completo[,1:34], 2, function(x) tapply(x, fila, mean))
